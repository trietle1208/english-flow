#!/usr/bin/env python3
"""Fast resume: translate remaining TSL glosses with MyMemory only (local IPA/POS)."""
...
def main() -> None:
    ...
    print(f"Selected {len(selected)}; cache has {len(cache)}", flush=True)

    pending = [w for w in selected if not cache.get(w, {}).get("meaning")]
    print(f"Pending translations: {len(pending)}", flush=True)

    for i, word in enumerate(pending, start=1):
        definition = defs[word]
        print(f"translate [{i}/{len(pending)}] {word}", flush=True)
        try:
            ...
            if i % 20 == 0:
                save_cache(cache)
                print(f"progress {i}/{len(pending)} total_cache={len(cache)}", flush=True)
            time.sleep(0.03)
