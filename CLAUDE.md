...
**Phase: 01–14 done (Phase 1 complete).** Drizzle + `pg` (node-postgres; switched from `postgres-js` on 2026-09-25 for one round trip per query — see `src/db/index.ts`; `scripts/*.mjs` still use `postgres`) + Zod-validated `src/env.ts`, `docker-compose.yml` (postgres + app + adminer, healthcheck, named `pgdata` volume), multi-stage `Dockerfile` (non-root `nextjs` user), `next.config.ts` `output: "standalone"`, `db:*` scripts. `npx tsc --noEmit` / `npm run lint` / `npm run build` / `npm run test` all pass (Node 18 and 22) and `src/env.ts` fails fast with a clear message on a missing var.
